...
// ─── Writeback ledger (close-alert / response-action targets) ────────────────
// Records every writeback the mock vendors accept so tests can verify what
// Atlas sent. Cleared on restart (and via POST /_debug/clear-writebacks).
const writebacks = [];

function recordWriteback(entry) {
  writebacks.push({ ...entry, at: new Date().toISOString() });
  return writebacks.length;
}

function getWritebacks(limit = 100) {
  return writebacks.slice(-limit);
}

function clearWritebacks() {
  writebacks.length = 0;
}
...
module.exports = {
  ...
  recordWriteback, getWritebacks, clearWritebacks
};
